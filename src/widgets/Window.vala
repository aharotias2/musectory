 ...
 */
 ...
 public class TatamWindow : Gtk.Window {
    ...
    public TatamWindow(Gtk.Application application, Tatam.Options options) {
    ...
    private void setup_widgets() {
                ...
                sidebar_popover = new Gtk.Popover(menu_button);
                {
                    sidebar = new Tatam.Sidebar();
                    {
                        ...
                        sidebar.playlist_del_button_clicked.connect((playlist_path) => {
                            sidebar_popover.visible = false;
                            return Tatam.Dialogs.confirm(Tatam.Text.CONFIRM_REMOVE_PLAYLIST, this);
                        });
        ...
        size_allocate.connect((cr) => {
            if (artwork_view.visible && controller.has_artwork()) {
                artwork_view.fit_image();
            }
            sidebar.max_width = get_allocated_width();
            sidebar.max_height = get_allocated_height();
