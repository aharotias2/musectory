...
namespace Musectory.Files {
    ...
    public Musectory.FileType get_file_type(string file_path) throws FileError {
        Musectory.FileType answer = Musectory.FileType.UNKNOWN;
        ...
        try {
            ...
            if (file_type == GLib.FileType.REGULAR) {
                return Musectory.FileType.FILE;
            }

            if (file_type == GLib.FileType.DIRECTORY) {
                answer = Musectory.FileType.DIRECTORY;
                ...
                dreader.file_found.connect((file) => {
                    try {
                        ...
                        if (finfo.get_content_type().has_prefix("audio/")) {
                            answer = Musectory.FileType.DISC;
                            return false;
                        } else {
                            return true;
                        }
    ...
    public void find_dir_files(
        string dir_path, out Gee.List<string> dir_list, out Gee.List<Musectory.FileInfo?> file_list
        ) throws Musectory.Error, FileError
    {
        FileInfoAdapter freader = new FileInfoAdapter();
        Gee.List<string> dir_list_local = new Gee.ArrayList<string>();
        Gee.List<Musectory.FileInfo?> file_list_local = new Gee.ArrayList<Musectory.FileInfo?>();
        if (FileUtils.test(dir_path, FileTest.IS_REGULAR)) {
            ...
            dreader.file_found.connect((file) => {
                try {
                    ...
                    if (mime_type.has_prefix("audio/")) {
                        Musectory.FileInfo? file_info = freader.read_metadata_from_path(file.get_path());
                        if (file_info != null && file_info.type == Musectory.FileType.MUSIC) {
    ...
    public Gee.List<Musectory.FileInfo?>? get_file_info_list_in_dir(string dir_path) {
        if (!GLib.FileUtils.test(dir_path, FileTest.IS_DIR)) {
            return null;
        }

        Gee.List<Musectory.FileInfo?> info_list = new Gee.ArrayList<Musectory.FileInfo?>();
        Gee.List<Musectory.FileInfo?> file_list = new Gee.ArrayList<Musectory.FileInfo?>();
    ...
    public Gee.List<GLib.File> get_children(string dir_path) throws Musectory.Error, FileError {
    ...
    public Gee.List<Musectory.FileInfo?> find_file_infos_recursively(string dir_path) throws FileError {
        FileInfoAdapter freader = new FileInfoAdapter();
        Gee.List<Musectory.FileInfo?> info_list = new Gee.ArrayList<Musectory.FileInfo?>();
        if (!GLib.FileUtils.test(dir_path, FileTest.IS_DIR)) {
            Musectory.FileInfo? finfo = freader.read_metadata_from_path(dir_path);
            info_list.add(finfo);
        } else {
            ...
            foreach (string file_name in file_list) {
                ...
                Musectory.FileInfo? finfo = freader.read_metadata_from_path(file_name);
                info_list.add(finfo);
            }
        }
        return info_list;
    ...
    public Gdk.Pixbuf? load_first_artwork(string dir_path, int size)
        ...
        dreader.file_found.connect((file) => {
            Musectory.FileInfo? file_info = mreader.read_metadata_from_path(file.get_path());
    ...
    public Musectory.FileInfo make_subdir_info(string dir_path, string subdir_path) {
        Musectory.FileInfo info = new Musectory.FileInfo();
        info.dir = dir_path.dup();
        info.path = subdir_path.dup();
        info.name = Path.get_basename(subdir_path);
        info.type = Musectory.FileType.DIRECTORY;
        return info;
    }
}
